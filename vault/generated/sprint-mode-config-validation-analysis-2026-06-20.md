...
## 2026-06-21 two-step sprint smoke run

Дата проверки: 2026-06-21 16:22-16:40 UTC.
...
- After manual repair, `tasks_status` showed sprint `#47` active with progress `2/2`; both child steps had state `merged` and `prUrl` set.
- Root issue `#47` remained open after both child PRs were merged.
- `finalPr` remained `null`, and there was no open final sprint PR from `sprint/sprint-two-step-test-sprint` to `development`.
...
2. Child auto-merge and finalization did not complete end-to-end.
   ...
   - The system did not independently move from child PR merge to final sprint completion.

3. Root sprint task does not close automatically.
   - This is one of the two user-observed known issues.
   - Even after progress reached `2/2` and both child issues were closed/done, root issue `#47` remained open.
   - `finalPr` stayed `null`.
   - Expected behavior for `reviewPolicy: sprint`: once all child steps are merged into the sprint branch, DevClaw should create or finalize the sprint PR to `development`, then close root issue `#47` after final merge/approval.

4. GitHub Relationships are incomplete for blocker dependencies.
   ...
   - GitHub Relationships show parent/root linkage only. They do not show a native relationship saying `Test sprint step 1` blocks `Test sprint step 2`.
   - This confirms that parent/root relationship projection and blocker dependency projection are not equivalent in the current implementation/runtime path.

5. PR bodies still use non-closing issue text.
   - PR `#50` and PR `#51` use `Addresses issue #48/#49`.
   - As noted in the earlier analysis, this does not create formal GitHub closing linkage in the Development/sidebar model.
   - If formal provider linkage is required, DevClaw needs either closing keywords such as `Fixes #...` / `Closes #...` or explicit provider API linkage.
...
5. Project native GitHub blocker relationships explicitly.
   - Keep parent/root linkage, but separately create a native blocked-by relationship from child `#49` to child `#48`.
